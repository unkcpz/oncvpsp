...
class OncvParser(BaseParser):
    ...
    @lazy_property
    def atan_logders(self) -> Dict[int, AePsNamedTuple]:
        """
        ...
        for x in [0, 20, 40, 60, 80, 100]:
            for l in l_list:
                tag = "!%7d%6d" % (l, x) if l >= 0 else "!     %d     %d" % (l, x)
                print(tag)
                ...
                ae_atan_logder_l[l] = AtanLogDer(l=l, energies=data[:, 2], values=data[:, 3])
                ps_atan_logder_l[l] = AtanLogDer(l=l, energies=data[:, 2], values=data[:, 4])

            d[x] = copy.deepcopy(AePsNamedTuple(ae=ae_atan_logder_l, ps=ps_atan_logder_l))
